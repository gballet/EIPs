...
```python3
# Create all access events given an instruction.
#   - address is the target of an instruction, so in the case of e.g. a CALL, this is the _callee_.
#   - caller only has meaning for *CALL instructions, and represents the address the call is sent
#     from.
def read_accesses_for_instruction(opcode: int, address: bytes20, caller: Optional[bytes20], key: Optional[bytes32]):
  match opcode:
    case OP_CALL | OP_DELEGATECALL | OP_STATICCALL | OP_CALLCODE | OP_SELFDESTRUCT | OP_EXTCODECOPY:
      if value_bearing or not (is_precompile(address) or is_system_contract(address)):
       add_read_access_event(address, 0, BASIC_DATA_LEAF_KEY)
      # For all opcodes that also have a caller, mark the caller's basic
      # data leaf as accessed.
      if opcode != OP_EXTCODECOPY:
        add_read_access_event(caller, 0, BASIC_DATA_LEAF_KEY)
    ...
    case OP_CREATE | OP_CREATE2:
      read_accesses_for_contract_creation()

# Create access events when a contract creation is initiated, either by
# a transaction or by a CREATEn instruction.
def read_accesses_for_contract_creation(target_address):
  add_read_access_event(address, 0, BASIC_DATA_LEAF_KEY)
  add_read_access_event(address, 0, CODEHASH_LEAF_KEY)
...
When:
 ...
 # XXX pas encore fait ce cas, parce que je l'ai pas compris
...
# XXX pas encore sur de ça non plus
...
def interpreter_loop(code: bytes, contract_address: bytes20):
  ...
  while True:
    if pc < len(code):
      code_read_access_event(pc, code[pc], contract_address)
      # ... rest of the interpreter loop

def code_read_access_event(pc: int, opcode: byte, contract_address):
  chunk = pc // CHUNK_SIZE
